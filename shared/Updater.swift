...
// アプリ内アップデート。apps/XiOSDesktop/Sources/Updater.swift を、どのアプリからも使えるよう
// 名前をバンドルから取る形に直したもの。project.yml の sources に `- ../../shared` を足せば載る。
//
// LiveContainer(3.7.2 以降)は、ゲストを開くたびに LCAppInfo.patchExecAndSignIfNeed を呼ぶ
// (LCAppModel.runApp)。そこでは LCAppInfo.plist の LCPatchRevision が現在値未満なら実行ファイルを
// 加工し直し、続けて署名もやり直す。だからこのアプリは
//   1. PC の配布サーバー(tools/serve-ipa.py)から新しい ipa を取り
//   2. Payload/<名前>.app を自分の .app(LiveContainer の Documents/Applications/<bundleId>.app)と入れ替え
//   3. 元の LCAppInfo.plist(データ置き場の UUID 等)を引き継いで LCPatchRevision を 0 に戻し
//   4. 終了する
// だけでよく、次に LiveContainer から開くと「加工 → 署名 → 起動」が自動で走る。
//
// ゲストからの livecontainer://install?url= は「再起動して入れてください」の警告を出すだけ
// (TweakLoader/UIKit+GuestHooks.m)なので、こちらの道にした。

/// Updater が使うログの口。各アプリのログ型をこれに合わせる。
protocol UpdaterLog: AnyObject {
    func log(_ s: String)
    var fileURL: URL { get }
    /// 書き込みを確定させる。書くたびに fsync している実装は何もしなくてよい
    func sync()
}
...
final class Updater: ObservableObject {
    ...
    /// 外出先向けは `tailscale cert` の正規証明書で https 化した口(port+1)。
    ...
    /// 配布物の名前。CI が作る ipa と manifest は実行ファイル名で並んでいる
    static var appName: String {
        (Bundle.main.infoDictionary?["CFBundleExecutable"] as? String) ?? "App"
    }
    /// 作業用フォルダの名前。LiveContainer では .app のフォルダ名が bundle id になっている
    private static var workPrefix: String {
        Bundle.main.bundleURL.standardizedFileURL.deletingPathExtension().lastPathComponent
    }
    ...
    func check() async {
        ...
        for s in servers {
            guard let url = URL(string: s + "/\(Self.appName).json") else { continue }
    ...
    func update() async {
        ...
        let ipaURL = tmpDir.appendingPathComponent("\(Self.appName).ipa")
        do {
            // 1. 取得
            phase = .downloading
            progress = 0
            ...
            let staging = parent.appendingPathComponent("\(Self.workPrefix)-update.tmp", isDirectory: true)
            let old = parent.appendingPathComponent("\(Self.workPrefix)-old.tmp", isDirectory: true)
    ...
    static func cleanup() {
        ...
        for n in ["\(workPrefix)-old.tmp", "\(workPrefix)-update.tmp"] {
    ...
    /// ログを配布サーバーの /upload/ に POST する(tools/serve-ipa.py do_POST)。
    ...
    func sendLog() async {
        ...
        for s in Self.defaultServers where !servers.contains(s) { servers.append(s) }
        for s in servers {
            guard let url = URL(string: s + "/upload/" + name) else { continue }
            var req = URLRequest(url: url)
            req.httpMethod = "POST"
