...
//   2. Payload/XLite.app を自分の .app(LiveContainer の Documents/Applications/
//      com.rutoi.xlite.app)と入れ替え
...
final class Updater: ObservableObject {
    ...
    func check() async {
        ...
        for s in servers {
            guard let url = URL(string: s + "/XLite.json") else { continue }
    ...
    func update() async {
        ...
        let ipaURL = tmpDir.appendingPathComponent("XLite.ipa")
        do {
            // 1. 取得
            phase = .downloading
            progress = 0
            ...
            let staging = parent.appendingPathComponent("xlite-update.tmp", isDirectory: true)
            let old = parent.appendingPathComponent("xlite-old.tmp", isDirectory: true)
    ...
    static func cleanup() {
        ...
        for n in ["xlite-old.tmp", "xlite-update.tmp"] {
    ...
    /// xlite.log を配布サーバーの /upload/ に POST する(tools/serve-ipa.py do_POST)。
    ...
    func sendLog() async {
        ...
        for s in Self.defaultServers where !servers.contains(s) { servers.append(s) }
        for s in servers {
            guard let url = URL(string: s + "/upload/xlite.log") else { continue }
            var req = URLRequest(url: url)
            req.httpMethod = "POST"
