# XiOSDesktop — 脱獄なしの iPhone で Linux デスクトップを動かす

LiveContainer のゲストとして、Linux デスクトップのユーザー空間を**翻訳なし・ネイティブ速度**で動かす。
実験用コンソールの [XiOSLite](../XiOSLite/README.md) から、画面を出す段で分岐したアプリ。
計画は `~/.claude/plans/wobbly-noodling-pony.md`、コンポジタとの通信仕様は
`tools/xios/iosc-host-protocol.md`、実機の計測値は `tools/xios/G0-results.md`。

## 仕組み(3 層)

1. **procd** — Linux の「プロセス」を「dylib を読んで `LC_MAIN` の main を別スレッドで呼ぶ」に
   置き換える。iOS はプログラムが別のプログラムを作ることを禁じているため
2. **pathmap** — バイナリに焼き込まれた `/var/jb` を、実行時にアプリ内のパスへ読み替える。
   `open`/`stat`/`opendir`/`readdir`/`fopen` などを横取りする
3. **xpcshim + xsurface** — コンポジタ `iosc` が要求する root の XPC サービスをプロセス内で肩代わりし、
   コンポジタが描いた IOSurface を受け取って Metal で画面に出す

## 画面の出し方

「画面」ボタン。コンポジタ `iosc` が居なければ起こし、ddx ソケットができるまで最大 10 秒待ってから
全画面に切り替える。右上の「コンソール」でログに戻れる。

## コンソール

コマンド入力欄に絶対パスで打って「Run」。例:

```
/var/jb/usr/bin/ls -la /var/jb/usr/bin
/var/jb/usr/bin/bash -c "for i in 1 2 3; do echo $i; done"
```

パイプ・リダイレクト・bash からの外部コマンド呼び出しは未実装(段階 G3)。
...
ログは画面の他に `Documents/xiosdesktop.log` に fsync 付きで残る(Copy / Share log ボタン)。
...
`libLCsys.dylib` は `apps/XiOSDesktop/native/*.c` + `native/*.m` から postbuild.sh が macOS ランナー上で
...
`apps/XiOSDesktop/stage.txt` があるので workflow は先に ubuntu の `stage` ジョブ
...
`.\tools\build.ps1 XiOSDesktop` → SharedFolder → LiveContainer。
