...
- **指の扱いを本家 Xios アプリと同じに**(XScreen.swift、classic iosc セッション): 指 1 本は
  ポインタ(押下は遅延: 静止長押し 0.55 秒で右クリック、動いたら起点で左押下、静止タップは
  離した時に押下+解放)、2 本以上は TOUCH。窓の移動・リサイズ・右クリックが指で効く。
  「指1本=ポインタ」トグルで従来の全 TOUCH に戻せる
...
- 入力ソケットへの書き込みで主スレッド(タッチ処理と描画が同居)を止めない: 送信バッファが
  埋まっているときは MOTION と TOUCH の移動を捨て(次の位置で上書きされる情報)、押下・離す・
  文字は 100ms まで待つ。送信バッファも 256KB に(実機 2026-09-14: Gears 描画中に指を動かすと
  止まって見えた疑い。捨てた数はログに出る)

- **合成が毎秒 20 回で頭打ち**(実機 2026-09-14: Gears 36FPS に対し DIRTY 15〜20/s)。iosc は PRESENTED の
  ack が返るまでクライアントに次のフレームを描かせず(present_ack_timer_cb)、こちらは画面に出た瞬間
  (垂直同期 1 回あと)に ack していた。GPU の読み取り完了(addCompletedHandler)で先に ack する。
  実測時刻つきの ack は従来どおり
- Gears は一覧・ドックから外す(NoDisplay。閉じる要求を無視する古いデモ)。コンソールの「歯車」で起こせる

- **面の読み取りと画面提示を分離**(実機 2026-09-14: ack 早期化後も合成は 25〜34 回/秒で頭打ち)。
  読み取りスレッドが DIRTY を垂直同期と無関係に受け、GPU で面を自前のテクスチャへ複写した完了で
  RELEASED と PRESENTED を返す。draw は最新の複写を 60fps で貼るだけ。クライアントの周回が
  垂直同期(16.7ms 刻み)から解放される。`xs_fd` を追加(poll(2) で待つ)

- build 59 の回転後に画面が更新されない(dirty 累計 0)。繋ぎ直し中は disconnected が true のままで、
  そこで起こした読み取りスレッドが即終了していた。ループの条件を世代番号だけにする

- **指の操作を iPadOS 流に**(瑠人さんの方針)。指は全部 wl_touch で GTK に渡す。GTK4 が慣性スクロール・
  ピンチ・長押しの文脈メニュー・文字選択ハンドルを自前でやる(docs/gnome-touch-ux.md「wl_touch alone」)。
  自前の 0.55 秒長押し右クリックの誤爆(実機 2026-09-14 に 30 秒で 12 回)が消える。
...
- **論理画面の幅を選べるようにした**(コンソールの「文字大きめ / 広く使う」)。864 は文字が最も大きい
  代わりに縦の作業領域が 286 しかなく、保存の画面のような縦長の窓が収まらない。1278 にすると
  作業領域が 526 になり収まるが、文字はおよそ 3 分の 2 になる。セッションを始める前に選ぶ
