...
nm -gU "$OUT" | grep -E ' _(open|stat|lstat|exit|_exit|fork|dlopen|realpath|posix_spawn|lcsys_init|lcsys_spawn|lcsys_wait|lcsys_install_xpc_shim|lcsys_shared_event_for_token|xs_connect|xs_poll|xs_release|xs_presented|xs_surface|xs_close|xi_connect|xi_touch|xi_text|xi_key|statfs|statvfs|fork|vfork|waitpid|wait4|kill|lc_savectx|lc_restorectx|lcsys_fork_child|xs_pacing|xs_fd|xi_traits|xi_output|mkstemp|mkostemp|mkdtemp|dbopen|getgrouplist|bind|connect)($|[$])' || true
